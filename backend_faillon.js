...
function exercise1() {
    //TODO
    return clients.sort(function (cli1, cli2) {
        if (cli1.rut > cli2.rut) {
            return 1;
          }
          if (cli1.rut < cli2.rut) {
            return -1;
...
function exercise2() {
    ...
    for(let i = 0; i < result.length; i++) {
        ...
        for(let j = 0; j < accounts.length; j++) {            
            if(result[i].id == accounts[j].clientId) {
                result[i].balance = result[i].balance + accounts[j].balance;
            }
        }
    }

    return result.sort(function(cli1, cli2) {
        return cli2.balance - cli1.balance;
...
function exercise3() {
    //TODO
    var clientsByBankObj = {};
    for(let i = 0; i < banks.length; i++) {
        var clientsByBankArray = [];

        for(let j = 0; j < accounts.length; j++) {                  
            if(banks[i].id == accounts[j].bankId) {
                clientsByBankArray.push(clients.find(obj => {
                    return obj.id === accounts[j].clientId
                }));
            }
        }
        
        clientsByBankObj[banks[i].name] = [...new Set(clientsByBankArray.sort(function (cli1, cli2) {
            if (cli1.name > cli2.name) {
                return 1;
              }
              if (cli1.name < cli2.name) {
                return -1;
...
function exercise4() {
    ...
    for(let i = 0; i < accounts.length; i++) {          
        if(accounts[i].bankId === 1) {
            clientesScotia.push(accounts[i]);
        }
    }

    var scotia = accounts.filter(function (account) {
        return account.bankId === 1;
    }).reduce((res, obj) => {
        res[obj.clientId] = { 
          total: (obj.clientId in res ? res[obj.clientId].total : 0) + obj.balance 
        }
        // res.push((obj.clientId in res ? res[obj.clientId].total : 0) + obj.balance);
        return res;
      }, {});
